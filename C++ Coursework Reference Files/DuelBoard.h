...
*/
class DuelBoard : public AWGameBoard {
public:
	...
	*/
	std::vector<std::unique_ptr<AWGameMove>> GetPossibleMoves() const override;
	...
	*/
	void ApplyMove(std::unique_ptr<AWGameMove> m) override;
	...
	*/
	inline const std::vector<std::unique_ptr<AWGameMove>> & GetMoveHistory() const {
		return mHistory;
	}
...
private:
	...
	std::vector<std::unique_ptr<AWGameMove>> mHistory;
	...
	inline bool InBounds(AWBoardPosition p) const { 
		return p.InBounds(BOARD_SIZE);
	}

	// Returns true if the given position contains the enemy of the given player.
	inline bool PositionIsEnemy(AWBoardPosition position, Player p) const {
	...
	inline Player GetPlayerAtPosition(AWBoardPosition position) const {
