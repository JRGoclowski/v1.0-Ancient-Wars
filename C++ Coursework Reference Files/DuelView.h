...
class DuelView : public AWGameView {
private:
	// An OthelloView shares ownership of an OthelloBoard object.
	std::shared_ptr<DuelBoard> mDuelBoard;
...
public:
	...
	DuelView(std::shared_ptr<DuelBoard> b) 
		: mDuelBoard(b) {
	}
	...
	std::unique_ptr<AWGameMove> ParseMove(const std::string &move) const override;
	...
	// LINE LIMIT: 3 lines.
	friend std::ostream& operator<< (std::ostream &lhs, const DuelView &rhs);
...
// LINE LIMIT: 1 line.
std::ostream& operator<<(std::ostream &lhs, const DuelMove &rhs);
