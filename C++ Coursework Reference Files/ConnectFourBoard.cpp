...
void ConnectFourBoard::CheckForWinner() {
	...
	for (auto pos : AWBoardPosition::GetRectangularPositions(mBoard.size(), mBoard[0].size())) {
...
vector<unique_ptr<AWGameMove>> ConnectFourBoard::GetPossibleMoves() const {
	vector<unique_ptr<AWGameMove>> moves;
...
void ConnectFourBoard::ApplyMove(unique_ptr<AWGameMove> move) {
...
const vector<unique_ptr<AWGameMove>>& ConnectFourBoard::GetMoveHistory() const {
	return mHistory;
}
